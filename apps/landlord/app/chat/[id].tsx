...
export default function LandlordChatScreen(): React.ReactElement {
    ...
    const initialize = async (): Promise<void> => {
      try {
        ...
        if (convo) {
          ...
          // Enrich with tenant info (other party for landlord is tenant)
          try {
            const tenantDoc = await firestore().collection('users').doc(convo.tenantId).get();
            if (!isMounted) return;
            if (tenantDoc.exists) {
